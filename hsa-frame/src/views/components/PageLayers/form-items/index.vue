...
export default {
  ...
  watch: {
    itemsDatas: {
      handler(newVal) {
        ...
        if (arr.length > 0) {
          const { codeType, codeTypeExt } = this.getCodeTypeAndCodeTypeExt(arr)
          // this.getCodeTableOptions(arr.map(item => item.codeKey).join(','), arr)
          this.getCodeTableOptions(codeType, codeTypeExt, arr)
        }
        // mouduleItems
  ...
  methods: {
    getCodeTableOptions(codeType, codeTypeExt, codeItems) {
      const parms = {
        codeType: codeType,
        codeTypeExt: codeTypeExt
      }
      getCodeTableDetailConvergence(parms).then(res => {
        ...
        this.$emit('codeTableOptionsChange', res.data)
      })
    },
    ...
    getCodeTypeAndCodeTypeExt(arr) {
      let codeType = ''
      const codeTypeExt = []
      const _this = this
      arr.forEach((k) => {
        if (_this.getDataType(k.codeKey) === 'Object') {
          codeTypeExt.push(k.codeKey)
        } else {
          codeType += k.codeKey + ','
        }
      })
