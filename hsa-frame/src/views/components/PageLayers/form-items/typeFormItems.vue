<template>
  ...
  >
    <span slot="label" v-html="item.label" />
    <div v-if="item.type === 'input'" class="form-item-box">
      <el-input
        v-model.trim="formDatas[item.prop]"
    ...
    <div v-else-if="item.type === 'dateRange'" class="form-item-box">
      ...
      <TimeRange
        ...
        :datetype="'date'"
        :value-format="item.valueFormat || 'yyyy-MM-dd'"
        :clearable="item.clearable === undefined ? false : item.clearable "
        :disabled="item.disabled"
        style="width:100%; height: 100%"
        :start-disabled="item.startDisabled || false"
        :end-disabled="item.endDisabled || false"
        :start-disabled-date="item.startDisabledDate || false"
        :end-disabled-date="item.endDisabledDate || false"
        v-bind="item.params"
        v-on="item.events || {}"
      />
    </div>
...
moment.locale('zh-cn')
import itemHandle from './itemHandle.js'
...
export default {
  components: {
    ...
    TimeRange: () => import('../../PageSelects/Common/TimeRange')
  },
  directives: { Unit },
  filters: {
    setDate(value) {
