...
颜色字体以及时间等cloumns传参参考 ContentHandle 组件 （date，mom-date，color-text）
 -->
<template>
  <section :class="['pf-table', sectionHeight, otherClass ]">
    <!-- 表头配置组件 -->
    <header-config
    ...
    >
      ...
      <!-- 表格内容部分 -->
      <template v-for="(item, index) in columnsNew">
        <!-- 多选框 -->
        <el-table-column
        ...
        <el-table-column
          ...
          :key="item.label + 'multistage' + index"
          :column-key="item.label + 'multistage'"
          :align="item.align || 'center'"
          :label="item.label || ''"
          :width="item.width || ''"
          :fixed="item.fixed"
        ...
        <table-column
          v-else
          :key="item.label + item.prop + index"
          :item="item"
...
export default {
  ...
  mixins: [TableExpand, TableSummaries, Drag],
  props: {
    data: { type: [Array, Object], default: () => [] }, // 列表数据
    columns: { type: Array, default: () => [] },
    height: { type: [Number, String], default: '100%' },
    showSummary: { type: Boolean, default: false }, // 显示合计
    ...
    cellStyle: { type: Function, default: new Function() }, // 表格设置单元格样式
    ...
    rowDrag: { type: Boolean, default: false }, // 行拖拽
    columnsDrag: { type: Boolean, default: false } // 列拖拽
  },
  data() {
    return {
  ...
  computed: {
    ...
    otherClass() {
      return this.showSummary ? '' : 'other-table'
    }
  },
  watch: {
    columns() {
      this.columnsNew = this.columns
      // getAllCloumns -> table-summaries 如果columns变化了，需要重新计算getAllCloumns
      this.getAllCloumns()
    },
    data(newVal) {
...
<style lang="scss" scoped>
  ...
  /deep/.el-table__empty-text{
    position: relative;
    background: transparent url(../../../assets/nodata.png) center no-repeat;
